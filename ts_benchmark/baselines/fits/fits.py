...
class FITS(ModelBase):
        ...
        if len(train_data.shape) == 3:
            ...
            if exog_dim > 0:
                ...
                if config.norm:
                    ...
                    train_exog = rearrange(
                        scaled_exog, "(l n) c -> l c n", l=train_data_l
                    )
                ...
                if config.norm:
                    ...
                    train_data = rearrange(
                        scaled_data, "(l n) c -> l c n", l=train_data_l
                    )
        else:
            # Handle 2D DataFrame format (original)
            ...
            if config.norm:
                if exog_dim > 0:
                    ...
                    series_scaled = self.scaler1.transform(
                        train_data.values[:, :series_dim]
                    )
                    exog_scaled = self.scaler2.transform(
                        train_data.values[:, series_dim:]
                    )
                ...
                if len(valid_data.shape) == 3 and exog_dim > 0:
                    ...
                    if exog_dim > 0:
                        ...
                        scaled_values = np.concatenate(
                            [series_scaled, exog_scaled], axis=1
                        )
            ...
            while answer is None or answer.shape[0] < horizon:
                ...
                for batch_data in test_data_loader:
                    if len(batch_data) == 2:
                        ...
                        input, target = input.to(self.device), target.to(self.device)
                        input_mark, target_mark = input_mark.to(
                            self.device
                        ), target_mark.to(self.device)

                    exog_future = target[:, -config.horizon :, series_dim:].to(
                        self.device
                    )
                    output, low = self.model(input)
    ...
    def batch_forecast(
        ...
        real_batch_size = (
            self.config.batch_size * input_np.shape[3]
            if len(input_np.shape) > 3
            else input_np.shape[0]
        )
        series_dim = (
        ...
        if exog_data is not None:
            exog_dim = (
                exog_data.shape[-2] if len(exog_data.shape) > 3 else exog_data.shape[-1]
            )
