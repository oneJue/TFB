...
class DUET(ModelBase):
    ...
    def single_forecasting_hyper_param_tune(self, train_data: np.ndarray):
        # For numpy array, we can't infer frequency, so use default
        self.config.freq = "h"
        ...
        **kwargs,
        ...
        if train_ratio_in_tv != 1:
            ...
            valid_dataset, valid_data_loader = forecasting_data_provider(
                valid_data,
                config,
                timeenc=0,  # Changed from 1 to 0 to match adapters
    ...
    def forecast(
        ...
        *,
        ...
        padded_data = np.zeros((padding_len, test.shape[1], test.shape[2]))
        test = np.concatenate([test, padded_data], axis=0)

        test_data_set, test_data_loader = forecasting_data_provider(
            test, config, timeenc=0, batch_size=1, shuffle=False, drop_last=False
        )
    ...
    def batch_forecast(
        ...
        if self.config.norm:
            # Only inverse transform series data with scaler1
            batch_size, horizon_len, _ = answers.shape
            answers_reshaped = answers.reshape(-1, answers.shape[-1])
            answers = self.scaler1.inverse_transform(answers_reshaped).reshape(
                batch_size, horizon_len, -1
            )
